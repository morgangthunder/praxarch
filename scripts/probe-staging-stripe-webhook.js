...
async function main() {
  ...
  const file = "/app/controllers/eventPayments/stripe/stripe.controller.js";
  const cmd = `bash -lc 'docker exec ${app} sh -c "sed -n 560,650p ${file}; echo ===; grep -rn stripe_pb /app/public /app/Front-end 2>/dev/null | head -10; grep -rn stripe_pb_key /app 2>/dev/null | head -10; grep -rn subscription_prod /app 2>/dev/null | head -10"'`;
