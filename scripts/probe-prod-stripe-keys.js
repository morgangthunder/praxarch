...
async function main() {
  const envPath = "/home/ubuntu/apps/master/bubbl_book/Back-end/.env";
  const names = await ssh(`grep -i stripe '${envPath}' | sed 's/=.*/=<redacted>/'`);
  console.log("=== key names in .env ===");
  console.log(names.trim() || "(none)");
  const ecosystem = await ssh("grep -i stripe /home/ubuntu/apps/ecosystem.config.js | head -20 | sed 's/:.*/: <redacted>/'");
  console.log("\n=== ecosystem.config.js ===");
  console.log(ecosystem.trim() || "(none)");
}
