...
async function main() {
  ...
  const cmd = `bash -lc 'docker exec ${app} sh -c "grep -r process.env.stripe /app 2>/dev/null | head -30; echo ---; grep -r stripe_wh /app 2>/dev/null | head -20; echo ---; grep -r stripe_pb /app 2>/dev/null | head -20; echo ---; grep -r subscription_price /app 2>/dev/null | head -20"'`;
