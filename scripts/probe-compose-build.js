...
async function main() {
  ...
  const { stdout } = await execFileAsync(
    "ssh",
    ["-i", keyPath, "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new", "-o", "UserKnownHostsFile=/dev/null", `ubuntu@${srv.ip}`, `grep -E 'build:|image:' ${root}/docker-compose.yaml | head -15; echo '---'; grep -E 'build:|image:' ${root}/docker-compose.yml | head -5`],
    { timeout: 60000 }
  );
  console.log(stdout);
