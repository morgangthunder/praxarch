...
async function main() {
  const coolifyEnv = "/data/coolify/applications/nf6adysipbutbwzslufhhoqg/.env";
  const out = await ssh(`bash -lc 'test -f ${coolifyEnv} && grep -i stripe ${coolifyEnv} | sed "s/=.*/=<redacted>/" || echo "(no coolify prod .env)"'`);
