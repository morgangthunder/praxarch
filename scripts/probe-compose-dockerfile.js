...
async function main() {
  ...
  const { stdout } = await execFileAsync(
    "ssh",
    ["-i", keyPath, "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new", "-o", "UserKnownHostsFile=/dev/null", `ubuntu@${srv.ip}`, `head -80 ${root}/docker-compose.yml; echo '---'; head -30 ${root}/Dockerfile`],
    { timeout: 60000 }
  );
  console.log(stdout);
