...
async function main() {
  ...
  const cmd = `bash -lc 'docker exec ${app} sh -c "grep -rn webhook.routes /app 2>/dev/null | head -10; grep -rn stripe_pb /app 2>/dev/null | head -15; grep -rn subscription_prod /app 2>/dev/null | head -15"'`;
