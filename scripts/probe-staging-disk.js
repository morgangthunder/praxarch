...
async function main() {
  ...
  const pk = keys.find((k) => k.uuid === (srv.private_key_uuid || srv.private_key_id) || k.id === (srv.private_key_uuid || srv.private_key_id))?.private_key;
  const keyPath = join(tmpdir(), "k");
  await writeFile(keyPath, pk, { mode: 0o600 });
  const cmd = "df -h /; echo '---'; sudo docker system df; echo '---'; sudo du -sh /var/lib/docker 2>/dev/null; sudo du -sh /data/coolify 2>/dev/null | head -3";
