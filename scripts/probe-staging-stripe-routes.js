...
async function main() {
  ...
  const cmd = `bash -lc 'docker exec ${app} sh -c "grep -rn stripeEventListener /app/routes 2>/dev/null; grep -rn stripe_pb_key /app 2>/dev/null | head -15; grep -rn subscription_prod_id /app 2>/dev/null | head -15; grep -rn stripe_subscription_price_id /app 2>/dev/null | head -10"'`;
