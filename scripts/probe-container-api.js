...
async function main() {
  ...
  const cmd = [
    ...
    "echo '=== routes files ==='",
    "docker exec $APP find /app -maxdepth 3 -name '*.js' | grep -E 'server|onboarding|provider' | head -20",
    "echo '=== api probes localhost 3303 ==='",
    "for path in /api/onboarding-agent/status /api/support-chat/status /api/health /api/version; do",
    "  body=$(curl -s http://127.0.0.1:3303$path -H 'Accept: application/json' | head -c 80);",
    "  echo \"$path => $body\";",
    "done",
  ].join("\n");
