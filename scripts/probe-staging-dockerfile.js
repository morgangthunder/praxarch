...
async function main() {
  ...
  const cmd = `head -120 ${root}/Dockerfile 2>/dev/null; echo '---'; ls -la ${root}/; echo '---'; du -sh ${root}/* 2>/dev/null | sort -hr | head -15; echo '---'; df -h /; swapon --show`;
  const { stdout } = await execFileAsync("ssh", ["-i", keyPath, "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new", "-o", "UserKnownHostsFile=/dev/null", `ubuntu@${srv.ip}`, cmd], { timeout: 60000, maxBuffer: 4*1024*1024 });
  console.log(stdout);
