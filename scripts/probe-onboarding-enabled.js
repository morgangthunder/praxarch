/** Verify onboarding agent enabled after GEMINI key added. */
...
async function main() {
  ...
  const cmd = [
    ...
    "echo app=$APP",
    "echo '=== coolify .env gemini ==='",
    "grep -E '^(GEMINI|LLM_|ONBOARDING)' /data/coolify/applications/gacc5qlsha4e9nrqk1vf58b3/.env | while IFS='=' read -r k v; do echo \"$k len=${#v}\"; done",
    "echo '=== container env ==='",
    "docker exec $APP env | grep -E '^(GEMINI|LLM_|ONBOARDING)' | while IFS='=' read -r k v; do echo \"$k len=${#v}\"; done",
    "echo '=== mcp health ==='",
    "curl -s http://127.0.0.1:3400/health",
    "echo",
    "echo '=== onboarding status (inside app) ==='",
    "docker exec $APP wget -qO- http://127.0.0.1:3303/api/onboarding-agent/status 2>/dev/null | head -c 500",
    "echo",
  ].join(" && ");
