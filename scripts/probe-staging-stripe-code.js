...
async function ssh(serverUuid, cmd) {
...
async function main() {
  ...
  const cmd = `bash -lc 'docker ps --format "{{.Names}}" | head -3; APP=$(docker ps --format "{{.Names}}" | grep gacc5 | head -1); echo app=$APP; docker exec "$APP" sh -c "grep -rn stripe_ /app --include=*.js 2>/dev/null | grep process.env | head -40"'`;
  console.log(await ssh("rorxx790bkr8db4ssro9v5fh", cmd));
}
